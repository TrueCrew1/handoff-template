...
const CONFIG = {
  ...
  "product": "True Crew Handoff Template",
  "boundary": "Production workflow state, customer data, product runtime, or company operating-system authority.",
...
const normalize = (v) => v.replace(/^git@github\.com:/,'').replace(/^https:\/\/github\.com\//,'').replace(/\.git$/,'');
...
const staged=list(git(['diff','--cached','--name-only'],true)), unstaged=list(git(['diff','--name-only'],true)), untracked=list(git(['ls-files','--others','--exclude-standard'],true));
const dirty=staged.length+unstaged.length+untracked.length>0;
const expectedBranch=process.env.TRUECREW_EXPECTED_BRANCH?.trim()||'', expectedHead=process.env.TRUECREW_EXPECTED_HEAD?.trim()||'', taskPacket=process.env.TRUECREW_TASK_PACKET?.trim()||'not supplied';
...
if(taskPacket==='not supplied') warnings.push('TRUECREW_TASK_PACKET was not supplied; reconcile scope/authority with the assigned packet or explicit owner instruction before mutation');
